...
function PupusaCard({ item, market, onAdd, sides }) {
  ...
  function toggleSide(id) {
    setSelectedSides(prev =>
      prev.includes(id)
        ? prev.filter(s => s !== id)
        : (prev.length < MAX_SIDES ? [...prev, id] : prev)
    );
  }
...
export default function Order() {
  ...
  const [pupusas, setPupusas] = useState([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // 2) ONLY trust server for open market — NO FALLBACK
        ...
        if (mRes.ok) {
          ...
          mkt = {
            id: m.marketId || m.id,
            name: m.name,
            date: new Date(m.startISO),
            start: new Date(m.startISO),
            end: new Date(m.endISO),
          };
        } else if (mRes.status === 404) {
          // Explicitly no market; keep mkt = null
        } else {
          const j = await mRes.json().catch(() => ({}));
          throw new Error(j.error || `Failed to load next-open (${mRes.status})`);
        }
        ...
        <section className="order-hero">
          <div className="container center">
            ...
            <p className="order-hero-subtitle">
              Online ordering is currently closed. Please check back the day before the next market.
  ...
  return (
    <main className="order-next">
      ...
      <section className="order-hero">
        <div className="container center">
          <span className="pill">Pickup at next open market</span>
          <h1 className="order-hero-title">Order</h1>
          <p className="order-hero-subtitle">
      ...
      <section className="section">
        <div className="container">
          <div className="row between align-center section-head">
            ...
            <div className="muted">Pick a pupusa and choose up to four sides.</div>
          </div>
              ...
              <article key={drink.id} className="card">
                ...
                <div className="card-body">
                  ...
                  <div className="card-row">
                    ...
                    <button className="btn btn-secondary" onClick={() => addDrink(drink)} disabled={!market}>
                      Add
                    </button>
                  </div>
                </div>
